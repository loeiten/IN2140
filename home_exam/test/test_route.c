...
void testCreateRoutingTables(void) {
  ...
  //   // FIXME: Make a choice whether one should use label or indices on the
  //   labels
  //   //        Easy could be to use a 1 to 1 map and use labels, or use a
  //   label
  //   //        like (i*10 + i/2) for all nodes except node 1
  //   //        NOTE: In general it's dangerous to rely on the index as there
  //   is no
...
#define N (6)
  ...
  int success = createRoutingTables(routeArrayC, &routingTable, N);
  assert(success == EXIT_SUCCESS);
